...
class NativeWin32InputController(InputController):
    ...
    def drag_screen_region_to_screen_region(self, button=MouseButton.LEFT, start_screen_region=None,
                                            end_screen_region=None, duration=1, **kwargs):
        if ("force" in kwargs and kwargs["force"] is True) or self.game_is_focused:
    ...
    def _interpolate_mouse_movement(start_windows_coordinates, end_windows_coordinates, steps=20):
        ...
        intermediate_x_coordinates = np.linspace(start_windows_coordinates[0], end_windows_coordinates[0], steps + 1)[
                                     1:]
...
for i in range(0,10):
    ...
    if keyboard_key_mapping[KeyboardKey.KEY_5.name] >= 1024:
        key = keyboard_key_mapping[KeyboardKey.KEY_5.name] - 1024
        flags = 0x0008 | 0x0001
    else:
        key = keyboard_key_mapping[KeyboardKey.KEY_5.name]
        flags = 0x0008
